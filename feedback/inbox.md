...
Meta-governance update: Add a layer 0 above the four-layer AI Workflow OS map. Future Skill discovery should explicitly evaluate metacognition governance, assumption auditing, priority ranking, agent orchestration, agent/tool fit, retrospective optimization, quality gates, complexity governance, and evidence tracking. This layer should decide whether a task is worth doing, what evidence is needed, how complex the solution should be, which agent or Skill should be used, and what validation is required before any automatic loop or production workflow begins.
